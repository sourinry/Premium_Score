...
const getOldMatches = async (req, res) => {
  try {
    const result = await matchService.getOldMatches(req.query);

    return res.status(200).json({
      success: true,
      message: "Old matches fetched successfully",
      count: result.matches.length,
      total: result.total,
      page: result.page,
      limit: result.limit,
...
const getMatchById = async (req, res) => {
  try {
    ...
    const match = await matchService.getMatchById(eventId);
